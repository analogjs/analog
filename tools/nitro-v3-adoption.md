...
| Direct Vite real Angular development                                   | With the srvx patch, repeated real Angular SSR returns 200 with rendered content. Client linking qualification is described below.                                                                                                                                                                                |
...
| Cloudflare development fixture                                         | With the env-runner patch, rendering, metadata, assets, HEAD and concurrent session checks pass; standalone runner regressions also prove POST/body/authorization fidelity.                                                                                                                                       |
...
- `xvfb-run -a pnpm nx run-many --target test --all`: 18 of 19 targets passed, including the headed browser; `docs-analog:test` has five suites failing with `SyntaxError: Invalid or unexpected token`. Other project targets passed.
...
Tracking issue: [analogjs/analog#2534](https://github.com/analogjs/analog/issues/2534). Draft PR: [analogjs/analog#2535](https://github.com/analogjs/analog/pull/2535), targeting `alpha` while qualification remains incomplete.
...
The real Angular development browser exposed an existing missing argument in two Rolldown compiler-plugin callers: the lifecycle boolean was interpreted as `isTest`, disabling linking. Both the default and experimental compilation-API paths now pass test mode independently. The Angular plugin suite passes 847 tests (6 existing skips), including four caller-mode regressions, and its build passes.

After rebuilding and clearing the fixture dependency optimizer cache, Chromium passed the real blog redirect and hydrated navigation to `/about` with no page errors. The production-only sitemap expectation remains outside development qualification.
