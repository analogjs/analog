...
// Tailwind `@reference` injection runs through the standalone `analog.setup()`
// plugin. TDBG_LIVE_RELOAD=false disables HMR-driven style externalization so
// the plugin path is exercised without it.
...
function resolvedConfigProbe(): Plugin {
  return {
    ...
    configResolved(config) {
      console.log(
        `[tdbg] compilationApi=${USE_COMPILATION_API} liveReload=${LIVE_RELOAD} oxc=${JSON.stringify((config as any).oxc)} esbuild=${JSON.stringify(config.esbuild)}`,
      );
    },
  };
}
