...
Development defaults to `experimental.componentStyleHmr: 'auto'`. Native stylesheet updates are enabled for the Compilation API with Angular 21–22/Vite 7–8, and for ngtsc with Angular 20–22 when an integration already requires external styles. These qualified external CSS updates preserve Emulated/None component DOM, focus, selection, and child state by updating Angular's existing stylesheet links. Ordinary ngtsc styles and the Compilation API on Vite 6 retain metadata updates: enabling external styles missed the template-latency gate in those combinations. Shared Sass/Less dependencies still use Vite's stylesheet pipeline. ShadowDom, missing stylesheet identity, and failed stylesheet loads retain correctness fallbacks. Angular 19 shared-resource edits reload because its metadata replacement can leave parent definitions stale.
