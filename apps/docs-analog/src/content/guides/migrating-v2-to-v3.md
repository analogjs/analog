...
Development defaults to `experimental.componentStyleHmr: 'auto'`. Native stylesheet updates are enabled for ngtsc with Angular 20–22/Vite 6.0, and the Compilation API with Angular 21–22/Vite 7–8. These qualified external CSS updates preserve Emulated/None component DOM, focus, selection, and child state by updating Angular's existing stylesheet links. Other combinations retain metadata updates: external styles missed the template-latency gate for ngtsc on Vite 6.4/7/8 and the Compilation API on Vite 6. Shared Sass/Less dependencies still use Vite's stylesheet pipeline. ShadowDom, missing stylesheet identity, and failed stylesheet loads retain correctness fallbacks. Angular 19 shared-resource edits reload because its metadata replacement can leave parent definitions stale.
