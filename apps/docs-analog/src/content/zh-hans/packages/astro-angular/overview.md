...
为了在与其他插件（如 [Starlight](https://starlight.astro.build)）集成时获得更好的兼容性，请将 Angular 组件放在特定文件夹中，并使用 `transformFilter` 选项仅转换这些文件。该集成会通过内部 Vite 插件的 `analog.setup()` 钩子将过滤器注册到 Angular。
