(() => {
  ...
  var b = (i, e) => {
    for (var t in e) w(i, t, { get: e[t], enumerable: !0 });
  };
  var f = {};
  b(f, {
    FilterPills: () => c,
  ...
  var g = (i) => {
    ...
    y = (i, e) => {
      let t = new r('li').class('pagefind-modular-list-result');
      if (e) {
        let l = new r('div').class('pagefind-modular-list-thumb').addTo(t);
        i?.meta?.image &&
          new r('img')
      ...
      let s = new r('div').class('pagefind-modular-list-inner').addTo(t),
    ...
    o = class {
      constructor(e) {
        if (
          ...
          (this.showImages = e.showImages ?? !0),
          e.containerElement)
        )
  ...
  var c = class {
    ...
    pillInner(e, t) {
      return this.total
        ? `<span aria-label="${e}" data-pfmod-pill-count="${t}">${e} (${t})</span>`
        ...
        this.instance.on('filters', (t) => {
          ...
          (this.available = Object.entries(s)),
            Array.isArray(this.ordering)
              ? this.available.sort((n, l) => {
                  let m = this.ordering.indexOf(n[0]),
                    _ = this.ordering.indexOf(l[0]);
                  return (m === -1 ? 1 / 0 : m) - (_ === -1 ? 1 / 0 : _);
                })
              : this.available.sort((n, l) => n[0].localeCompare(l[0])),
