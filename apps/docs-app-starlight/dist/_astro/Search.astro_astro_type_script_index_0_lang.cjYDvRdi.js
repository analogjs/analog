    ...
    if (l && l.length > 0) {
      ...
      h = o(
        l.map((n) => {
          ...
          if (
            ((e.rel = a ? 'stylesheet' : y),
            a || (e.as = 'script'),
            (e.crossOrigin = ''),
            (e.href = n),
            u && e.setAttribute('nonce', u),
...
class v extends HTMLElement {
  constructor() {
    ...
    const m = this.querySelector('button[data-open-modal]'),
      l = this.querySelector('button[data-close-modal]'),
      c = this.querySelector('dialog'),
      h = this.querySelector('.dialog-frame'),
      d = (e) => {
        ('href' in (e.target || {}) ||
          (document.body.contains(e.target) && !h.contains(e.target))) &&
          t();
      },
      o = (e) => {
        c.showModal(),
    ...
    m.addEventListener('click', o),
      ...
      window.addEventListener('keydown', (e) => {
        (e.metaKey === !0 || e.ctrlKey === !0) &&
          e.key === 'k' &&
          (c.open ? t() : o(), e.preventDefault());
      });
    let u = {};
    try {
      u = JSON.parse(this.dataset.translations || '{}');
    } catch {}
    const r =
    ...
    window.addEventListener('DOMContentLoaded', () => {
      (window.requestIdleCallback || ((i) => setTimeout(i, 1)))(async () => {
        const { PagefindUI: i } = await S(async () => {
          const { PagefindUI: s } = await import('./ui-core.D_Lfcn_I.js');
          return { PagefindUI: s };
        }, []);
        new i({
          ...E,
          ...
          processResult: (s) => {
            (s.url = r(s.url)),
              (s.sub_results = s.sub_results.map(
                (p) => ((p.url = r(p.url)), p),
              ));
          },
        });
