...
- a scoped `llms.txt` is generated for each multi-page section from the same route records
- `llms-full.txt` is generated by concatenating the Markdown source files under `apps/docs-app/docs`
- a per-page `.md` file is emitted for each docs page alongside the built HTML
