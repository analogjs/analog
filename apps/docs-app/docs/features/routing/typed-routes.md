...
When enabled, the build generates a `src/routeTree.gen.ts` file that augments `AnalogRouteTable` with typed params and query for each file-based route. This is similar to TanStack Router's `routeTree.gen.ts` codegen.
...
| `AnalogFileRoutesByFullPath` | Routes indexed by resolved navigation path       |
| `AnalogFileRoutesByTo`       | Routes indexed by navigation target              |
| `AnalogRouteTreeId`          | Union type of all route ids                      |
| `AnalogRouteTreeFullPath`    | Union type of all full paths                     |
| `AnalogRouteTreeTo`          | Union type of all navigation targets             |
| `analogRouteTree`            | Runtime object with `byId`, `byFullPath`, `byTo` |
...
The route tree is useful for structure-aware tooling such as breadcrumb generation, sidebar navigation, route analysis, or build-time manifests:
...
This metadata surface is additive — projects that only use `routePath()` and `injectTypedRouter()` can ignore it.
