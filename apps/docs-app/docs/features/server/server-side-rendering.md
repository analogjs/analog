...
When using Vite 8+ with Rolldown as the bundler, you can control how client-side chunks are created using the `codeSplitting` option. This is useful for optimizing bundle sizes by grouping vendor or shared modules into separate chunks.
...
| --------- | ---------------------- | -------------------------------------------- |
| `groups`  | `CodeSplittingGroup[]` | Define custom chunk groups                   |
...
| Option          | Type                                             | Description                                                |
| --------------- | ------------------------------------------------ | ---------------------------------------------------------- |
| `name`          | `string \| ((moduleId: string) => string\|null)` | Chunk name or function returning a chunk name              |
| `test`          | `RegExp`                                         | Pattern to match module IDs for this group                 |
