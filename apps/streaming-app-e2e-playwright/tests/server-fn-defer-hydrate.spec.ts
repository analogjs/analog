...
describe('serverFn inside @defer (hydrate on immediate)', () => {
  ...
  test('streamed deferred blocks hydrate and respond to interactions', async () => {
    ...
    try {
      ...
      expect(await response?.text()).toContain('data-analog-authoritative');
      await page.locator('.btn-a').click();
      await expect
        .poll(() => page.locator('.block-a').textContent())
        .toContain('42');
      await page.locator('.btn-b').click();
      await expect
        .poll(() => page.locator('.block-b').textContent())
        .toContain('winter');
  ...
  test('an interrupted document shows an error without bootstrapping Angular', async () => {
    ...
    page.on('pageerror', (error) => errors.push(error.message));
    try {
      await page.route('**/interrupted-render', (route) =>
        route.fulfill({
          contentType: 'text/html',
          body: html.slice(0, tail),
        }),
      );
      await page.goto(`${baseURL}/interrupted-render`, {
        waitUntil: 'networkidle',
      });
      expect(await page.title()).toBe('Unable to load this page');
      expect(
        await page.locator('meta[name="robots"]').getAttribute('content'),
      ).toBe('noindex');
      expect(
        await page.locator('[data-analog-render-error]').textContent(),
      ).toContain('The page could not finish loading.');
