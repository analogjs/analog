...
export function findImageDomains(
  ...
  return (context: ts.TransformationContext) => {
    return (sourceFile: ts.SourceFile) => {
      ...
      function isImageProviderValue(
        property: ts.ObjectLiteralElementLike
      ): boolean {
        return (
          ts.isPropertyAssignment(property) &&
          property.name.getText() === 'useValue'
        );
      }
      ...
      function findProvidersAssignment(node: ts.Node) {
        if (ts.isPropertyAssignment(node)) {
          if (
            ...
            node.name.escapedText === 'providers'
          ) {
      ...
      function findFeaturesAssignment(node: ts.Node) {
        if (ts.isPropertyAssignment(node)) {
          if (
            ...
            const providerElement = node.initializer.elements.find(
              isProvidersFeatureElement
            );
            if (
              providerElement &&
      ...
      function findPropertyDeclaration(node: ts.Node) {
        if (
          ...
          if (node.name.escapedText === 'ɵinj') {
            ts.visitEachChild(
              node.initializer.arguments[0],
              findProvidersAssignment,
              context
            );
          } else if (node.name.escapedText === 'ɵcmp') {
            ts.visitEachChild(
              node.initializer.arguments[0],
              findFeaturesAssignment,
              context
            );
          }
        }
