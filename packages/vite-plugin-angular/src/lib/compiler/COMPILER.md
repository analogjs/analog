...
When `liveReload` is enabled, Angular emits component styles as external stylesheets (`externalRuntimeStyles`) loaded via `<link>` URLs that carry `ngcomp` and `e` query params. Vite compiles those files first (SCSS/Sass/Less → CSS → `const __vite__css = "..."`). A dedicated post plugin (`analogjs-encapsulate-component-styles`) then runs Angular's `encapsulateStyle` on the compiled CSS so `:host` / `:host-context` become valid emulated-encapsulation selectors. This cannot live in the main TypeScript transform: that hook's filter is TS-only, and encapsulation must run _after_ preprocessor compilation.
