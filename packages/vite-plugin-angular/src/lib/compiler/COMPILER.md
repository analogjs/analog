...
| `js-emitter.ts`       | Angular output AST → JavaScript string emitter (~4x faster than `ts.Printer`), handles OXC/TS WrappedNodeExpr, bare module refs (`ngImport: i0`), `$localize`, native optional chaining (`isOptional` → `?.`/`?.[ ]`/`?.()`) |
...
| FESM build optimizer (`elideMetadata`, `wrapStaticMembers`, `markPure`, `adjustEnums`) | ⚠️ Falls back to Analog's `JavaScriptTransformer`-backed optimizer — OXC's `optimizeAngularPackage` doesn't tree-shake `@angular/core/fesm2022/core.mjs` (adds 401 B instead of eliding metadata + dead JIT branches), inflating the client bundle ~35%                       |
...
| `fastCompileMode: 'partial'` (library publishing)                                      | ✅ OXC emits `ɵɵngDeclare*` partial declarations and returns `dtsDeclarations`; an `enforce: 'post'` plugin splices the Ivy `.d.ts` members into the emitted declarations. Base `.d.ts` come from a separate generator (rolldown-plugin-dts / vite-plugin-dts / tsdown / tsc) |
...
### Open OXC drift on `0.0.31`

No known output divergences from Angular's reference compiler on `0.0.31`.

The earlier signal-query divergences — separate-statement query emission (`fn(...); fn(...)` instead of chained `fn(...)(...)`) and reversed `_c<N>` const-table indices for content vs view queries — were fixed upstream in [voidzero-dev/oxc-angular-compiler#323](https://github.com/voidzero-dev/oxc-angular-compiler/pull/323) and landed in `0.0.31`. The `signal_queries / query_in_component` fixture in `oxc-upstream-parity.spec.ts` now asserts parity (`expectedDivergent: { oxc: false }`).
...
| `oxc-optimizer-plugin.ts` | Vite plugin wrapping `optimizeAngularPackage`. Production-only (`apply: 'build'`); FESM filter; sets `ngDevMode` / `ngJitMode` / `ngI18nClosureMode` / `ngServerMode` defines. **Not currently mounted** — both engines run through Analog's `JavaScriptTransformer`-backed optimizer pending upstream tree-shaking improvements |
...
| `js-emitter.spec.ts`            | 31    | Operator precedence/associativity and parenthesization, nullish-coalescing grouping, assignment wrapping in ternary conditions, member-access receiver parenthesization, native optional chaining (`isOptional` → `?.`/`?.[ ]`/`?.()`, incl. `errors()?.email` newsletter regression)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
