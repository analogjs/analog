...
2. The `ɵhmr_` callback copies Angular static metadata to the original live class and keeps its component definition and factory targeting that class.
...
Root components can also receive metadata updates without re-bootstrapping. Instance state is retained, but Angular recreates views; DOM state and child view state can change. Metadata replacement does not copy ordinary JavaScript methods from the donor class, so fast-mode class-behavior edits can require a manual reload. Non-Angular files use Vite's default HMR.

External templates, styles and imported Sass/Less dependencies invalidate every owning `.ts` module. Ownership is replaced when resource references change. Angular 19.0.0 reloads around its upstream HMR runtime bug; native HMR requires 19.0.1 or newer. Disabled HMR and JIT retain their compatibility paths.
