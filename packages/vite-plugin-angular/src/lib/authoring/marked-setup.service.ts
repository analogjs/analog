...
export class MarkedSetupService {
  ...
  constructor() {
    ...
    renderer.codespan = (text) => {
      const code = this.escapeBreakingCharacters(text.raw);
      return `<code>${code}</code>`;
    };

    renderer.paragraph = (text) => {
      if (
        this.detectAngularComponent(text.raw) ||
        this.detectAngularControlFlow(text.raw)
      ) {
        return this.decodeHtmlEntities(text.raw.trim());
      }
      return `<p>${text}</p>`;
    };
