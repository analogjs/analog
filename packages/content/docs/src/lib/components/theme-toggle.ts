 ...
 *
 * Both icons are rendered; CSS hides whichever doesn't match the .dark
 * class on <html>. Consumers should set the class pre-bootstrap (e.g.
 * via an inline script in index.html) so first paint has the correct
 * theme — no Angular state or hydration switch, so no flicker.
 */
@Component({
