...
export function toContentFilesRecord(
  allFiles: Record<string, unknown>,
  contentFilesList: ContentFile[],
  ...
  Object.entries(allFiles).forEach((entry) => {
    ...
    if (newFilename !== undefined) {
      ...
      objectUsingSlugAttribute[objectFilename] = value as () => Promise<string>;
    }
  });
...
>('@analogjs/content Content Files', {
  ...
  factory() {
    ...
    return toContentFilesRecord(
      Object.keys(files).length
        ? files
        : (analogEsbuildContentMaps().contentFiles ?? {}),
      inject(CONTENT_FILES_LIST_TOKEN),
    );
  },
});
