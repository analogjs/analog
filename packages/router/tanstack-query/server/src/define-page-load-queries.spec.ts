...
function makeCtx(
  ...
  return {
    ...
    // `definePageLoad` reads the URL from `event.web.request` when the
    // event carries a web request, which is the path used here.
    event: {
      web: { request: new Request(url), url: new URL(url) },
      node: { req: {}, res: {} },
    } as never,
  };
}
