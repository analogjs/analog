    ...
    if (seenByFullPath.has(fullPath)) {
      console.warn(
        `[Analog] Route collision: '${fullPath}' is defined by both ` +
          `'${seenByFullPath.get(fullPath)}' and '${filename}'. Skipping duplicate.`,
      );
      continue;
    }
...
/**
 * Generates a metadata-oriented route tree module for Angular tooling.
 */
export function generateRouteTreeDeclaration(
  ...
  lines.push('}');
  lines.push('');
  lines.push('export interface AnalogFileRoutesByTo {');
  for (const route of manifest.routes) {
    lines.push(
      `  ${toTsKey(route.fullPath)}: AnalogFileRoutesById[${toTsStringLiteral(route.id)}];`,
    );
  ...
  lines.push('export type AnalogRouteTreeTo = keyof AnalogFileRoutesByTo;');
  lines.push('');
...
// --- JSON-LD utilities ---
// Shared between the runtime (@analogjs/router json-ld.ts) and build-time
// (@analogjs/vite-plugin-routes json-ld-manifest-plugin.ts) so that
// normalisation logic doesn't drift between packages.
