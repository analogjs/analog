 ...
 * - `output` schema validates the response in development and tests only. Logs a warning on mismatch.
...
>(
  ...
  return async (event: H3Event): Promise<Response> => {
    ...
    const requestData =
      options.input || (options.body && method !== 'GET' && method !== 'HEAD')
