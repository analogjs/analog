...
export default defineConfig(({ mode }) => {
  return {
    ...
    build: {
      target: 'esnext',
      outDir: '../../node_modules/@analogjs/platform',
      emptyOutDir: true,
      sourcemap: true,
      minify: false,
      lib: {
        // Use an object entry so the key ('src/index') becomes the chunk [name].
        // This ensures the output path matches package.json exports ("./src/index.js")
        // across all platforms, including Windows where rolldown may derive
        // different chunk names from string entries with preserveModules.
    ...
    cacheDir: '../../node_modules/.vite/platform',
    test: {
      reporters: ['default'],
      globals: true,
      environment: 'node',
