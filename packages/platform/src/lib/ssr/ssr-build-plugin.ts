  ...
  const plugins: Plugin[] = [
    {
      ...
      // Filtered transform: only files whose path matches one of these
      // three third-party libraries enter the handler.
      transform: {
        filter: {
          id: /zone-node|platform-server|domino\/lib\/sloppy\.js/,
        },
        handler(code, id) {
          if (
