...
import { filenameToRoutePath } from '@analogjs/router/manifest';
...
export function generateJsonLdManifestSource(
  ...
  lines.push('type JsonLdObject = Record<string, unknown>;');
  lines.push(
    "type GeneratedJsonLdManifestEntry = { routePath: string; sourceFile: string; kind: 'module' | 'content'; resolveJsonLd: () => JsonLdObject[]; };",
  );
  lines.push('');
  lines.push('function normalizeJsonLd(value: unknown): JsonLdObject[] {');
  lines.push('  if (Array.isArray(value)) {');
  lines.push(
    "    return value.filter((entry): entry is JsonLdObject => typeof entry === 'object' && entry !== null && !Array.isArray(entry));",
  );
  lines.push('  }');
  lines.push('');
  lines.push(
    "  return typeof value === 'object' && value !== null ? [value as JsonLdObject] : [];",
  );
  lines.push('}');
  lines.push('');
  lines.push(
  ...
  entries.forEach((entry) => {
    if (entry.kind === 'module') {
      lines.push(
        `  ['${entry.routePath}', { routePath: '${entry.routePath}', sourceFile: '${entry.sourceFile}', kind: 'module', resolveJsonLd: () => normalizeJsonLd(${entry.importAlias}.routeJsonLd ?? ${entry.importAlias}.routeMeta?.jsonLd) }],`,
      );
      return;
    }
